...
export default function LaboratoryWorkList({ laboratoryWorks }: { laboratoryWorks: CompleteLaboratoryWork[] }) {
  const { data: l } = trpc.laboratoryWorks.getLaboratoryWorks.useQuery(undefined, {
  ...
  return (
    <ul>
      {l.laboratoryWorks.map((laboratoryWork) => (
        <LaboratoryWork laboratoryWork={laboratoryWork} key={laboratoryWork.laboratoryWork.id} />
      ))}
    </ul>
  );
}

const LaboratoryWork = ({ laboratoryWork }: { laboratoryWork: CompleteLaboratoryWork }) => {
  return (

import { getGroupSubjectById, getGroupSubjects } from "@/lib/api/groupSubjects/queries";
...
import { createGroupSubject, deleteGroupSubject, updateGroupSubject } from "@/lib/api/groupSubjects/mutations";

export const groupSubjectsRouter = router({
  getGroupSubjects: publicProcedure.query(async () => {
    return getGroupSubjects();
  }),
  getGroupSubjectById: publicProcedure.input(groupSubjectIdSchema).query(async ({ input }) => {

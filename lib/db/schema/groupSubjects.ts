...
export const groupSubjects = pgTable('group_subjects', {
  ...
  subjectId: varchar("subject_id", { length: 256 }).references(() => subjects.id).notNull(),
  groupId: varchar("group_id", { length: 256 }).references(() => groups.id).notNull(),
  hours: integer("hours"),
...
export const insertGroupSubjectSchema = createInsertSchema(groupSubjects).omit(timestamps);
export const insertGroupSubjectParams = baseSchema.extend({
...
export type CompleteGroupSubject = Awaited<ReturnType<typeof getGroupSubjects>>["groupSubjects"][number];

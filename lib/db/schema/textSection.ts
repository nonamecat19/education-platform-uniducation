...
export const textSection = pgTable('text_section', {
  ...
  unitId: varchar("unit_id", { length: 256 }).references(() => units.id).notNull(),
...
export const insertTextSectionSchema = createInsertSchema(textSection).omit(timestamps);
export const insertTextSectionParams = baseSchema.extend({
...
export type CompleteTextSection = Awaited<ReturnType<typeof getTextSection>>["textSection"][number];

...
import { type TextSectionId, textSectionIdSchema, textSection } from "@/lib/db/schema/textSection";
...
export const getTextSectionById = async (id: TextSectionId) => {
  ...
  const [row] = await db.select({ textSection: textSection, unit: units }).from(textSection).where(eq(textSection.id, textSectionId)).leftJoin(units, eq(textSection.unitId, units.id));

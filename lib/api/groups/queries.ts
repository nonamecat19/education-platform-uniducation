...
export const getGroups = async () => {
  const rows = await db.select({ group: groups, teacher: teachers }).from(groups).leftJoin(teachers, eq(groups.teacherId, teachers.id));
...
export const getGroupById = async (id: GroupId) => {
  ...
  const [row] = await db.select({ group: groups, teacher: teachers }).from(groups).where(eq(groups.id, groupId)).leftJoin(teachers, eq(groups.teacherId, teachers.id));

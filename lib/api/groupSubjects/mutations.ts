...
export const createGroupSubject = async (groupSubject: NewGroupSubjectParams) => {
  const newGroupSubject = insertGroupSubjectSchema.parse(groupSubject);
  try {
    const [g] =  await db.insert(groupSubjects).values(newGroupSubject).returning();
...
export const updateGroupSubject = async (id: GroupSubjectId, groupSubject: UpdateGroupSubjectParams) => {
...
export const deleteGroupSubject = async (id: GroupSubjectId) => {
  const { id: groupSubjectId } = groupSubjectIdSchema.parse({ id });
  try {
    const [g] =  await db.delete(groupSubjects).where(eq(groupSubjects.id, groupSubjectId!))
    .returning();
    return { groupSubject: g };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";

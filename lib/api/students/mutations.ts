...
export const updateStudent = async (id: StudentId, student: UpdateStudentParams) => {
  ...
  try {
    const [s] =  await db
     ...
     .where(and(eq(students.id, studentId!), eq(students.userId, session?.user.id!)))
     .returning();
    return { student: s };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
...
export const deleteStudent = async (id: StudentId) => {
  ...
  const { id: studentId } = studentIdSchema.parse({ id });
  try {
    const [s] =  await db.delete(students).where(and(eq(students.id, studentId!), eq(students.userId, session?.user.id!)))
    .returning();
    return { student: s };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
